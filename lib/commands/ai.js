...
import {
  inquirerConfirmPrompt,
  inquirerInputPrompt,
  inquirerPasswordPrompt,
  ...
  try {
    if (!password) {
      const { apiKey } = await inquirer.prompt(
        inquirerPasswordPrompt(AI_PROMPT_NAME, AI_PROMPT_MESSAGE)
      );
...
async function askAI() {
  ...
  try {
    const { aiQuestion } = await inquirer.prompt(
      inquirerInputPrompt(AI_ASK_PROMPT_NAME, AI_ASK_PROMPT_MESSAGE)
    );
    ...
    const { confirmation } = await inquirer.prompt(
      inquirerConfirmPrompt(AI_ASK_CONFIRM_MESSAGE)
    );
