...
export async function gitCommands() {
  printTitle(config.GIT_TITLE);

  try {
    const cmd = await processList(
      config.GIT_LIST_PROMPT_NAME,
      config.GIT_LIST_PROMPT_MESSAGE,
    ...
    switch (cmd) {
      case config.GIT_OPTION_ADD_ALL:
        gitAddAll();
        return;
      case config.GIT_OPTION_LIST_BRANCHES:
        gitListBranches();
        return;
      case config.GIT_OPTION_COMMIT:
        gitCommit();
        return;
      case config.GIT_OPTION_CREATE_BRANCH:
        gitCreateBranch();
        return;
      case config.GIT_OPTION_PUSH_TO_CURRENT_BRANCH:
        pushToCurrentBranch();
        return;
    }
  } catch (err) {
...
async function gitAddAll() {
  try {
    ...
    executeCommand(
      config.GIT_COMMAND,
      config.GIT_ADD_ARGS,
      config.GIT_ADD_SUCCESS_MESSAGE,
      config.GIT_ADD_FAILED_MESSAGE
    );
  } catch (err) {
    handleErrors(err);
  }
...
async function gitCommit() {
  try {
    ...
    executeCommand(
      config.GIT_COMMAND,
      [...config.GIT_COMMIT_ARGS, input],
      config.GIT_COMMIT_SUCCESS_MESSAGE,
      config.GIT_COMMIT_FAILED_MESSAGE
    );
  } catch (err) {
    handleErrors(err);
  }
...
async function gitCreateBranch() {
  try {
    ...
    executeCommand(
      config.GIT_COMMAND,
      [...config.GIT_CHECKOUT_ARGS, input],
      config.GIT_CREATE_BRANCH_SUCCESS_MESSAGE,
      config.GIT_CREATE_BRANCH_FAILED_MESSAGE
    );
  } catch (err) {
    handleErrors(err);
  }
